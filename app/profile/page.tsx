...
export default async function ProfilePage() {
  ...
  const [user, orders, addresses] = await Promise.all([
    prisma.user.findUnique({
      ...
      select: { id: true, name: true, email: true, image: true, phone: true, createdAt: true, role: true },
    }),
    ...
    prisma.address.findMany({
      ...
      select: { id: true, fullName: true, phone: true, addressLine1: true, addressLine2: true, city: true, state: true, postalCode: true, country: true, isDefault: true },
    }),
  ])
          ...
          <CardContent className="space-y-3">
            {addresses.length === 0 ? (
              ...
              <div key={a.id} className="rounded border p-3">
                <div className="flex items-center justify-between">
                  <div className="font-medium">{a.fullName} {a.isDefault && <span className="ml-2 text-xs text-primary">(Default)</span>}</div>
                  <Button variant="ghost" size="sm" asChild><Link href={`/profile/address/${a.id}`}>Edit</Link></Button>
                </div>
            ...
            <Button size="sm" asChild><Link href="/profile/address/new">Add address</Link></Button>
          </CardContent>
        </Card>
      </div>
          ...
          {orders.length === 0 ? (
            ...
            <div key={o.id} className="flex items-center justify-between border rounded p-3">
              <div>
                ...
                <div className="text-sm text-muted-foreground">{o.items.length} items • {formatDate(o.createdAt)}</div>
              </div>
              <div className="flex items-center gap-4">
                <span className="text-sm">{o.status}</span>
                <span className="font-medium">{formatPrice(Number(o.totalAmount))}</span>
                <Button variant="outline" size="sm" asChild><Link href={`/orders/${o.id}`}>View</Link></Button>
              </div>
            </div>
          ))}
        </CardContent>
